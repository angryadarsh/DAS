    ...
    <?php $form = ActiveForm::begin(['id' => 'docter-form']); ?>
      ...
      <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
        ...
        <div class="w-full">
          ...
          <?= $form->field($model, 'specialization')->textInput(['class' => $inputCss ,'placeholder' => 'specialization','readonly' => true])->label(false) ?>
        </div>

        <div class="w-full">
          <label for="item-weight" class="block mb-2 text-sm font-medium text-gray-900">Qualification</label>
          <?= $form->field($model, 'qualification')->textInput(['class' => $inputCss ,'placeholder' => 'Qualification','readonly' => true])->label(false) ?>
        </div>

        <div class="w-full">
          <label for="item-weight" class="block mb-2 text-sm font-medium text-gray-900">Experience (in years)</label>
          <?= $form->field($model, 'experience')->textInput(['class' => $inputCss ,'placeholder' => 'experience','readonly' => true])->label(false) ?>
        </div>
    </div>
